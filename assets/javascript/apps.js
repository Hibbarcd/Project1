var config = { //Firebase Configuration Information
...
database.ref('/users').orderByChild("dateAdded").on("value", function (snapshot) {
    ...
    snapshot.forEach(ss => {
        if (!data.includes(ss.val())) { data.push(ss.val()); } //If unique value add to data array
    })
...
async function searchAPIs(query) {
    ...
    } else $('#contentDiv').html(`<h1>There were no results found for ${query}!</h1>`);
}
/*
